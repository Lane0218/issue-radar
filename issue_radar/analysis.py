...
def build_ai_prompts(issue_bundle: dict[str, Any], profile: dict[str, Any]) -> tuple[str, str]:
    ...
    system_prompt = (
        ...
        "Use the profile carefully when judging fit. "
        "If the profile prefers bounded, well-scoped tasks with a clear implementation path, "
        "then issues centered on crash/assertion/segfault/root-cause investigation or unclear debugging "
        "should usually be poor_fit and often too_hard or unclear unless the issue already makes the fix path concrete. "
